...
try {
const d = await fetch(`${API}/api/live`, {headers}).then(r=>r.json());
...
useEffect(()=>{
fetchAll(); fetchTrades(selAlgo); fetchOpenPnl(); fetchLive();
...
const i4=setInterval(fetchLive,10000);
...
<style>{`*{box-sizing:border-box;margin:0;padding:0} html,body{overflow-x:hidden;width:100%;-webkit-text-size-adjust:100%} ::-webkit-scrollbar{width:2px} ::-webkit-scrollbar-thumb{background:${BORDER}} @keyframes glow{0%,100%{opacity:1}50%{opacity:.45}} @keyframes blink{0%,100%{opacity:1}50%{opacity:.1}} .btn{cursor:pointer;border:none;font-family:monospace;transition:all .15s;-webkit-tap-highlight-color:transparent} a{color:inherit;text-decoration:none;-webkit-tap-highlight-color:transparent}`}</style>
...
```
  ...
  <div style={{display:"flex",background:CARD,borderBottom:`1px solid ${BORDER}`,overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
    {[["live","💰 LIVE"],["lab","LAB"],["trades","TRADES"],["compare","COMPARE"],["debug","DEBUG"],["settings","⚙"]].map(([v,l])=>(
  ...
  {view==="live"&&(
    <div style={{flex:1,padding:"10px 12px",overflowY:"auto"}}>
      {/* Wallet Balance */}
      <div style={{background:CARD,border:`1px solid ${G}33`,borderRadius:10,padding:"12px",marginBottom:10,borderLeft:`3px solid ${G}`}}>
        <div style={{fontSize:7,color:G,letterSpacing:3,marginBottom:8}}>WALLET BALANCE</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:6,color:DIM,marginBottom:2}}>SOL</div>
            <div style={{fontSize:18,fontWeight:900,color:"white"}}>{liveData?num(liveData.solBalance).toFixed(4):"--"}</div>
          </div>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:6,color:DIM,marginBottom:2}}>USD VALUE</div>
            <div style={{fontSize:18,fontWeight:900,color:G}}>${liveData?num(liveData.usdBalance).toFixed(2):"--"}</div>
          </div>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:6,color:DIM,marginBottom:2}}>LIVE P&L</div>
            <div style={{fontSize:18,fontWeight:900,color:liveData&&num(liveData.totalLivePnl)>=0?G:R}}>{liveData?fmt$(liveData.totalLivePnl):"--"}</div>
          </div>
        </div>
        <div style={{marginTop:8,display:"flex",justifyContent:"space-between",fontSize:7,color:DIM}}>
          <span>SOL price: ${liveData?num(liveData.solPrice).toFixed(2):"--"}</span>
          <span>Bet: ${liveData?liveData.betSize:"--"} per trade</span>
          <span style={{color:liveData?.recentErrors?.length?R:G}}>{liveData?.recentErrors?.length?"⚠ "+liveData.recentErrors.length+" errors":"✓ Connected"}</span>
        </div>
      </div>

      {/* Live Trade Summary */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
        <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:10,padding:"10px",textAlign:"center"}}>
          <div style={{fontSize:6,color:DIM,marginBottom:4}}>OPEN TRADES</div>
          <div style={{fontSize:24,fontWeight:900,color:Y}}>{liveData?liveData.openTrades:0}</div>
        </div>
        <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:10,padding:"10px",textAlign:"center"}}>
          <div style={{fontSize:6,color:DIM,marginBottom:4}}>CLOSED TRADES</div>
          <div style={{fontSize:24,fontWeight:900,color:"white"}}>{liveData?liveData.closedTrades:0}</div>
        </div>
      </div>

      {/* Live Open Trades */}
      {liveData&&Object.values(liveData.trades||{}).flat().filter(t=>t.live_status==="open").length>0&&(
        <div style={{marginBottom:10}}>
          <div style={{fontSize:7,color:G,letterSpacing:2,marginBottom:6}}>OPEN POSITIONS</div>
          {Object.entries(liveData.trades||{}).map(([k,arr])=>
            arr.filter(t=>t.live_status==="open").map(t=>(
              <div key={t.id} style={{background:CARD,border:`1px solid ${ALGO_COLORS[k]}44`,borderRadius:8,padding:"10px",marginBottom:6,borderLeft:`3px solid ${ALGO_COLORS[k]}`}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <div>
                    <span style={{fontSize:11,fontWeight:900,color:"white"}}>{t.ticker}</span>
                    <span style={{fontSize:7,color:ALGO_COLORS[k],marginLeft:6}}>{ALGO_NAMES[k]}</span>
                  </div>
                  <div style={{fontSize:9,color:Y}}>LIVE</div>
                </div>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:7,color:DIM}}>
                  <span>Bet: ${t.bet_size}</span>
                  <span>Entry: ${num(t.live_entry_price||t.entry_price).toFixed(8)}</span>
                  <span>Slip: {t.live_slippage_pct?`${num(t.live_slippage_pct).toFixed(2)}%`:"--"}</span>
                </div>
                {t.live_tx_buy&&(
                  <div style={{marginTop:4,fontSize:6,color:DIM}}>
                    <span>TX: </span>
                    <a href={`https://solscan.io/tx/${t.live_tx_buy}`} target="_blank" rel="noreferrer"
                      style={{color:B,textDecoration:"none"}}>{t.live_tx_buy.slice(0,16)}...</a>
                  </div>
                )}
              </div>
            ))
      ...
      {/* Live Closed Trades */}
      {liveData&&Object.values(liveData.trades||{}).flat().filter(t=>t.live_status==="closed").length>0&&(
        <div style={{marginBottom:10}}>
          <div style={{fontSize:7,color:DIM,letterSpacing:2,marginBottom:6}}>CLOSED POSITIONS</div>
          {Object.entries(liveData.trades||{}).map(([k,arr])=>
            arr.filter(t=>t.live_status==="closed").map(t=>(
              <div key={t.id} style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:8,padding:"10px",marginBottom:6,borderLeft:`3px solid ${num(t.live_pnl)>=0?G:R}`}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <div>
                    <span style={{fontSize:11,fontWeight:900,color:"white"}}>{t.ticker}</span>
                    <span style={{fontSize:7,color:ALGO_COLORS[k],marginLeft:6}}>{ALGO_NAMES[k]}</span>
                  </div>
                  <div style={{fontSize:14,fontWeight:900,color:num(t.live_pnl)>=0?G:R}}>{fmt$(t.live_pnl)}</div>
                </div>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:7,color:DIM}}>
                  <span>Bet: ${t.bet_size}</span>
                  <span>Slip: {t.live_slippage_pct?`${num(t.live_slippage_pct).toFixed(2)}%`:"--"}</span>
                  <span style={{color:num(t.live_pnl)>=0?G:R}}>{t.exit_reason}</span>
                </div>
                {t.live_tx_sell&&(
                  <div style={{marginTop:4,fontSize:6,color:DIM}}>
                    <a href={`https://solscan.io/tx/${t.live_tx_sell}`} target="_blank" rel="noreferrer"
                      style={{color:B,textDecoration:"none"}}>View on Solscan ↗</a>
                  </div>
                )}
              </div>
            ))
      ...
      {/* No trades yet */}
      {liveData&&liveData.openTrades===0&&liveData.closedTrades===0&&(
        <div style={{textAlign:"center",padding:"40px 20px",color:DIM,fontSize:9}}>
          <div style={{fontSize:16,marginBottom:8}}>🎯</div>
          <div>Bot is scanning for signals...</div>
          <div style={{fontSize:7,marginTop:4}}>First real trade will appear here</div>
        </div>
      )}

      {/* Recent Errors */}
      {liveData?.recentErrors?.length>0&&(
        <div style={{background:"#1a0a0a",border:`1px solid ${R}33`,borderRadius:8,padding:"10px"}}>
          <div style={{fontSize:7,color:R,letterSpacing:2,marginBottom:6}}>RECENT ERRORS</div>
          {liveData.recentErrors.slice(0,3).map((e,i)=>(
            <div key={i} style={{fontSize:6,color:DIM,marginBottom:3}}>
              <span style={{color:R}}>[{e.algo?.toUpperCase()}] </span>{e.err?.slice(0,60)}
            </div>
          ))}
        </div>
      )}

      <button onClick={()=>{fetchLive();fetchAll();}} style={{width:"100%",padding:"8px",background:"transparent",border:`1px solid ${BORDER}`,color:DIM,borderRadius:8,fontSize:9,letterSpacing:2,marginTop:10}}>↻ REFRESH</button>
    </div>
  )}
