...
// Per-pair history maps (shared — same market data)
...
// ── BET SIZING (per algo) ──────────────────────────────────
function betSize(sc, fomo, isStealth, algoKey) {
  const cfg  = ALGOS[algoKey];
  ...
  return Math.min(150, Math.max(25, Math.round((base * scoreMult * stealthMult) / 5) * 5));
}
...
async function insertTrade(algoKey, p, sc, fomo) {
  ...
  const bet       = betSize(sc, fomo, isStealth, algoKey);
