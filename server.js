      ...
      try {
        ...
        if (!pair) {
          ...
          if (ageMin > 30) {
            ...
            dailyPnl += pnl;
            await notify(`DELISTED: ${t.ticker}`, "Pair vanished. Closed -50%.");
          }
          continue;
        }
...
app.get("/health", (req, res) => res.json({
  status: "ok", ts: new Date().toISOString(), version: "5.2",
...
app.get("/api/debug", async(req, res) => {
  try {
    const rows = (await db(`
      SELECT ticker, score, fomo_score, liq, vol_5m, pc_5m, age_min,
             entered, skip_reason, seen_at
      FROM signals ORDER BY seen_at DESC LIMIT 50`
    )).rows;
...
app.get("/api/fomo-feed", async(req, res) => {
  try {
    ...
    const r = await db(`
      SELECT ticker, pair_address, dex_url, score, fomo_score, price,
             vol_5m, liq, pc_5m, age_min, market_mood, entered, skip_reason, seen_at
      FROM signals
      WHERE seen_at > NOW() - INTERVAL '10 minutes'
        AND fomo_score >= ${threshold}
